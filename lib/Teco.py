...
def TecoGAN(r_inputs, r_targets, FLAGS, GAN_Flag=True):
    ...
    if(GAN_Flag): # Build the discriminator loss
        ...
        with tf.device('/gpu:0'), tf.variable_scope('tdicriminator_train'):
            tdiscrim_tvars = tf.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='tdiscriminator')
    ...
    with tf.variable_scope('generator_train'):
        ...
        gen_tvars = tf.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='generator')
        fnet_tvars = tf.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='fnet')
        ...
        if(not GAN_Flag):
            ...
            # tf.compat.v1.GraphKeys.UPDATE_OPS: batch normalization layer in discriminator should update first
